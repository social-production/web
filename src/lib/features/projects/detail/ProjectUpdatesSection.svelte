<script lang="ts">
  ...
  $: if (!votesRenderedInHub && showGovernanceVotes && data.viewerCanVoteOnUpdateRequests && !updateVotesManuallyCollapsed) {
    if (pendingUpdateVotes.length > 0) {
      showUpdateVotes = true;
    }
  }

  $: if (!votesRenderedInHub && showGovernanceVotes && data.viewerCanVoteOnEditRequests && !editVotesManuallyCollapsed) {
    if (pendingEditVotes.length > 0) {
      showEditVotes = true;
    }
  }
  ...
  $: canProposeUpdate = canRequestUpdate && (!showGovernanceVotes || data.updateRequests.length === 0);
  $: canProposeEdit = canRequestEdit && (!showGovernanceVotes || data.editRequests.length === 0);
...
<section class="updates-shell" id="updates">
  ...
  {#if !votesRenderedInHub && showGovernanceVotes && showUpdateVotes && data.updateRequests.length > 0}
  ...
  {#if !votesRenderedInHub && showGovernanceVotes && showEditVotes && data.editRequests.length > 0}
